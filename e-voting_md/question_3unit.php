...
if(document.form1.unit[i].checked){
total =total +1;}
if(total > 3){
alert("Please Select only three") 
document.form1.unit[j].checked = false ;
return false;
}
}
} </script>
  ...
  <div class="form-group col-sm-6">
  <label> Choose any 3 units:</label><br>
   <input type="checkbox" name="unit[]" value="1" onclick='chkcontrol(0)';> Unit 1
   <input type="checkbox" name="unit[]" value="2" onclick='chkcontrol(1)';> Unit 2
   <input type="checkbox" name="unit[]" value="3" onclick='chkcontrol(2)';> Unit 3
   <input type="checkbox" name="unit[]" value="4" onclick='chkcontrol(3)';> Unit 4
   <input type="checkbox" name="unit[]" value="5" onclick='chkcontrol(4)';> Unit 5
  </div>
  <div class="form-group col-sm-6">
  ...
  <div class="form-group">
    <input type="submit" name="sub" class="btn btn-primary" value="Proceed" style="float:right">
							...
							$u1_2m=getQuestions(3,'2m',$unit[0],$sub_code);
							
							$u2_2m=getQuestions(3,'2m',$unit[1],$sub_code);
							$u3_2m=getQuestions(4,'2m',$unit[2],$sub_code);
							/* $u4_2m=getQuestions(2,'2m',4,$sub_code);
							$u5_2m=getQuestions(2,'2m',5,$sub_code); */
							
								/* $two_marks=array_merge($u1_2m,$u2_2m,$u3_2m,$u4_2m,$u5_2m); */
								$two_marks=array_merge($u1_2m,$u2_2m,$u3_2m);
								$two=implode(",",$two_marks);
								/* print_r($two_marks); */
								/* 13 marks */
								
								$u1_13m=getQuestions(3,'13m',$unit[0],$sub_code);	
								$u2_13m=getQuestions(3,'13m',$unit[1],$sub_code);	
								$u3_13m=getQuestions(4,'13m',$unit[2],$sub_code);	/* 
								$u4_13m=getQuestions(2,'13m',4,$sub_code);	
								$u5_13m=getQuestions(2,'13m',5,$sub_code); */
								
								$thirteen_marks=array_merge($u1_13m,$u2_13m,$u3_13m);
								...
								if($aq==1)
								{
								$u1_15m=getQuestions15(2,'15m',$unit[0],$sub_code,0);	
								$u2_15m=getQuestions15(2,'15m',$unit[1],$sub_code,1);	
								$u3_15m=getQuestions15(2,'15m',$unit[2],$sub_code,0);	
								}
								else
								{
								$u1_15m=getQuestions15(2,'15m',$unit[0],$sub_code,1);	
								$u2_15m=getQuestions15(2,'15m',$unit[1],$sub_code,0);	
								$u3_15m=getQuestions15(2,'15m',$unit[2],$sub_code,1);	
								}	
								
								$fivteen_marks=array_merge($u1_15m,$u2_15m,$u3_15m);
...
$unit1=implode(",",$unit);
echo "<a href='quest_bank.php?sub_code=$sub_code&author=$sid&cat=3units&unit=$unit1&t1=$two&t2=$thirteen&t3=$fifteen&question_title=$question_title&date=$date' style='float:right;margin:20px' class='btn btn-success'>Save</a>";


}
else{
	echo "<script>alert('Choose only 3 units')</script>";
}
}
